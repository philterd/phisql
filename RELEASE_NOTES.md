...
- **`region` on the phone number filter** (RFC #22). A new optional property on `filterPhoneNumber`, a single ISO 3166-1 alpha-2 region string or an array of them (default `"US"`), that sets the default region(s) used to interpret phone numbers written without an international `"+"` country code, so national-format numbers from those locales are detected (numbers with a `"+"` prefix are found regardless). An array is scanned per region and the matches merged. Expressible today through the existing filter `OPTIONS` passthrough, `OPTIONS (region = 'GB')` or `OPTIONS (region = ['US', 'GB', 'FR'])`; no grammar change was required. Example `phone-region`.
...
- **`MAP_REPLACE` strategy** (RFC #30). A new filter strategy that replaces a detected value with a caller-supplied value from a lookup table, for deterministic and consistent pseudonyms (for example always turning `Smith` into `Jones`). The table is carried inline with `mappings` (an object of value/replacement pairs) and/or loaded from local files listed in `mappingFiles`; `caseSensitive` controls key matching. A value not in the table is handled by an optional `generator` (see below) and then by `fallbackStrategy` (default `REDACT`), so a detected value is never left in the clear. `MAP_REPLACE` is added to the `strategy` enum and its fields to `baseFilterStrategy` in schema `1.2.0`, plus a `MAP_REPLACE` keyword in the grammar. The arguments are set through the existing strategy-argument passthrough: `WITH MAP_REPLACE(mappings = ('Smith' = 'Jones'), fallbackStrategy = 'REDACT')`. Example `map-replace`.
- **Top-level `generators` block and `DEFINE GENERATOR` statement** (RFC #30). A named, reusable generator declared once at the top level and referenced by name from a `MAP_REPLACE` strategy's `generator` argument, to produce a replacement for a detected value absent from the lookup table (for example rewriting a business name while preserving its descriptor). Generators target a local model endpoint inside the deployment boundary (`type: "ollama"` first), and a required `timeoutMs` bounds the call so it can never block the pipeline. A new `generators` object (name-keyed map) is added to the schema root, a `generator` definition to `$defs`, and a `DEFINE GENERATOR '<name>' TYPE '<type>' OPTIONS ( ... )` statement to the grammar. The `endpoint` and `model` option keys are quoted because they are reserved keywords. Example `generator-fallback`.
- **`spec/v1.2.0/examples/map-replace` and `generator-fallback`** example pairs (`.phisql` and compiled `.json`).
- **`color` on filter strategies** (RFC #31). A new optional string on `baseFilterStrategy` and `dateFilterStrategy` (so every strategy inherits it) that sets the color of the redaction bar drawn over the spans a strategy redacts when rendering a PDF or image, so a single policy can color redactions by entity type or by condition. Per span the color resolves to the redacting strategy's `color`, then the policy-wide `config.pdf.redactionColor`, then black. It accepts a named color (`black`, `white`, `red`, `orange`, `yellow`, `green`, `blue`, `gray`) or a 6-digit `#RRGGBB` hex string; any unrecognized or malformed value renders black. It has no effect on text redaction. Expressible today through the existing strategy-argument passthrough, `WITH REDACT(color = 'red')`; no grammar change was required. Example `pdf-strategy-color`.
- **`spec/v1.2.0/examples/pdf-strategy-color`** example pair (`.phisql` and compiled `.json`).
- **`EIN` entity type** (RFC #33). A new first-class identifier for the U.S. Employer Identification Number (federal tax ID, format `NN-NNNNNNN`), the business counterpart to `SSN`. Adds an `ein` identifier and a `filterEin` definition to schema `1.2.0` and an `EIN` row to the entity-types catalog, so `REDACT EIN WITH ...` compiles to an `ein` filter with an `einFilterStrategies` array. `filterEin` carries an optional `onlyValidPrefixes` boolean (default `false`) that, when true, restricts matches to EINs whose two-digit prefix is one the IRS issues, cutting false positives on format-valid but non-issued numbers. It is set through the existing filter `OPTIONS` passthrough (`OPTIONS (onlyValidPrefixes = TRUE)`), like `creditCard.onlyValidCreditCardNumbers`, so no grammar change was required. Example `ein`.
- **`spec/v1.2.0/examples/ein`** example pair (`.phisql` and compiled `.json`).

### Changed

- The current schema version advances to `1.2.0`. The three reference implementations target it (`redaction.policy.schema.version`, `SUPPORTED_SCHEMA_VERSION`, `PolicySchema.SupportedSchemaVersion`), and `validate_spec.py` validates and coverage-checks against `1.2.0`. The conformance runner's accept-case schema check (`compliance/run.py`) is bumped from `1.1.0` to `1.2.0` to match, as its own comment requires it to track `validate_spec.py`.
- **`namedArg` accepts a few keyword tokens as argument names.** So `MAP_REPLACE`'s `generator` argument (and `type`) can be written without quoting, the grammar's strategy-argument name now also accepts the `GENERATOR` and `TYPE` keyword tokens in addition to `ID`. The matched token's text is used unchanged, so `WITH MAP_REPLACE(generator = 'namer')` compiles to the `generator` property.
- **The two existing color fields now document the shared color set** (RFC #31). The descriptions of `config.pdf.redactionColor` and `graphical.boundingBoxes[].color` in schema `1.2.0` now state the same named-color set and 6-digit hex form as the new strategy `color`, and the black fallback for an unrecognized value. Description-only; no structural or behavioral change.
...
- EIN detection is a Phileas runtime behavior. The schema and catalog declare the `ein` entity; matching `NN-NNNNNNN`, disambiguating bare nine-digit runs against SSN, and honoring `onlyValidPrefixes` with the IRS-issued prefix set are implemented in Phileas separately (philterd/phileas#325, with the .NET and Python ports in philterd/phileas-dotnet#64 and philterd/phileas-python#53).
- Strategy `color` is a Phileas render behavior. The schema field declares the intent; drawing the bar in that color, mapping the named-color set and hex to RGB, and falling back to black for an unrecognized value are implemented in Phileas separately (philterd/phileas#324), with the .NET and Python ports tracked in philterd/phileas-dotnet#63 and philterd/phileas-python#52. The same color set and fallback govern `config.pdf.redactionColor` and `graphical.boundingBoxes[].color`, which share one color table in Phileas.
- The phone `region` field declares the intent; the actual detection (using the region as libphonenumber's default region, and scanning once per region for the array form) is implemented in the Phileas runtime separately. Multiple regions raise false positives, so a validity/leniency option may follow.
...
- `MAP_REPLACE` and the `generators` block declare the policy contract; the runtime behavior is implemented in Phileas separately, and identically across the Java, Python, and .NET ports. That includes map load and precedence (inline `mappings` over `mappingFiles`), case handling, invoking the generator against its local endpoint, the timeout and terminal `fallbackStrategy`, treating generator output as untrusted (reject output equal to the input and re-scan it for reintroduced PII), and caching so a repeated value is not re-generated. The compiler enforces unique generator names within a policy (`reject/semantic/duplicate-generator`); the reference compilers do not otherwise validate a generator `endpoint`, `model`, or that a referenced `generator` name exists, which the runtime resolves. A generator must target an endpoint inside the deployment boundary; no secrets belong in the policy.
