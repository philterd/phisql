...
- **Optional top-level `metadata` object** (#23). A policy can now carry descriptive information about itself: `metadata.description` is defined as a string, and additional properties are allowed so the object can grow (author, timestamps, labels) without a breaking schema change. Nothing in `metadata` affects detection or redaction; an engine reads and writes it unchanged. It exists so a tool's policy description travels with the policy through export, import, and sharing instead of living in that tool's own storage. The rest of the policy root stays strict, so a misspelled key is still rejected.

- **`DESCRIPTION` now compiles to `metadata.description`.** The clause on a `POLICY` declaration previously had nowhere to go in the policy JSON, because the schema had no description field: the compiler returned the text on `CompileResult` and a caller who wanted to keep it wrote a sibling `<basename>.md`. It is now written into the policy itself, so a description survives export, import, and sharing. `CompileResult.description` is unchanged, so callers that keep a description elsewhere keep working. All three references emit it; the compiled JSON for a policy without a `DESCRIPTION` clause is unchanged (no empty `metadata` object). The four `spec/v1.0/examples` policies that declare a description now carry it in their expected JSON.
