...
The schema is published at `https://philterd.ai/schemas/redaction-policy/<version>/schema.json` and is the canonical source. Phileas downloads it at build time (the version is the `redaction.policy.schema.version` Maven property in `pom.xml`) and embeds it in the jar, so the runtime reads the same schema it was built against. The schema is no longer stored in this repository.
